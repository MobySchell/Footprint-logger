...
export default function Track() {
  ...
  useEffect(() => {
    const total = emissions.reduce((sum, emission) => sum + emission.value, 0);
  ...
  const pieChartData = useMemo(() => {
    ...
    return {
      ...
      datasets: [
        {
          data: values,
          backgroundColor: categories.map((cat) => colors[cat] || "#CCCCCC"),
          borderColor: categories.map((cat) => colors[cat] || "#CCCCCC"),
          borderWidth: 2,
        },
      ],
    };
  ...
  useEffect(() => {
    if (activeFilter === "" || activeFilter === "Clear") {
      ...
      setFilteredEmissions(
        emissions.filter((emission) => emission.category === activeFilter)
      );
    }
  ...
  const clearAllEmissions = async () => {
    if (window.confirm("Are you sure you want to clear all emissions?")) {
      ...
      if (user?.id) {
        try {
          await fetch(`http://localhost:5000/api/emissions/${user.id}/all`, {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${localStorage.getItem("token")}`,
            },
          ...
          console.error("Error clearing emissions from database:", error);
        }
      }
      ...
      <div className="container mx-auto px-4">
        <div className="headingAndInputArea mt-[120px]">
          ...
          <div className="cardArea grid outline-solid rounded-2xl px-[50px] py-[30px] mt-[50px] gap-5">
            ...
            {/* User input */}
            <div id="manual-input" className="inputContainer grid grid-rows-2">
              <label
            ...
            <div id="running-total" className="block">
              <div className="bg-green-100 border border-green-400 rounded-xl p-4 mt-4">
                ...
                <p className="text-green-600">
                  {filteredEmissions.length} activities tracked
          ...
          <div className="bg-white rounded-xl p-6 shadow-[5px_5px_0px_1px_rgba(0,0,0,15)]">
            ...
            {emissions.length > 0 ? (
                  ...
                  <div className="space-y-2">
                    {Object.entries(
                      filteredEmissions.length > 0
                        ? filteredEmissions.reduce((acc, emission) => {
                            acc[emission.category] =
                              (acc[emission.category] || 0) + emission.value;
                            return acc;
                          }, {})
                        : emissions.reduce((acc, emission) => {
                            acc[emission.category] =
                              (acc[emission.category] || 0) + emission.value;
                            return acc;
                          }, {})
              ...
              <div className="text-center text-gray-500 py-8">
                <p className="text-lg">No emissions data to display</p>
                <p className="text-sm">
                  Start adding activities to see your carbon footprint
                  breakdown!
                </p>
              </div>
            )}
